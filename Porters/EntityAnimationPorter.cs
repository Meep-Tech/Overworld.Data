/*namespace Overworld.Data.IO {
  public partial class Entity {
    public partial class Animation {*/
      ...
      /// </summary>
      /*public class Porter : ArchetypePorter<Entity.Animation> {
        ...
        /// </summary>
        internal IEnumerable<Data.Entity.Animation.Type> LoadAllAnimationDatas(string rootFolder) {
          string entityAnimationsRootFolder = Path.Combine(
            rootFolder,
            MetadataFilePath
          );

          IEnumerable<Data.Entity.Animation.Type> entityAnimationsInRootFolder =
        ...
        /// <returns></returns>
        internal Data.Entity.Animation.Type LoadAnimationTypeFromFolder([NotNull] string folderName, string entityAnimationsRootFolder = "") {
          ...
          if(config.TryGetValue("version", out JToken versionToken)) {
            ...
            if(config.ContainsKey("name") && config.ContainsKey("namespace")) {
              ...
              if((clip = Resources.Load<AnimationClip>(getAnimationFileName())) != null) {
                return new Data.Entity.Animation.Type( config, clip, null);
              }
            }
          } else {
          ...
          Data.Entity.Animation.Layer layer = config.TryGetValue("layer", out JToken layerToken)
            ? layerToken.Value<Data.Entity.Animation.Layer>()
            : (config["layer"] = JToken.FromObject(Data.Entity.Animation.Layer.BaseBody)).Value<Data.Entity.Animation.Layer>();

          /// Step four, get tags
          IEnumerable<Data.Entity.Animation.Tag> tags = config.TryGetValue("tags", out JToken tagArray)
            ? tagArray.ToObject<IEnumerable<Data.Entity.Animation.Tag>>(Models.DefaultUniverse.ModelSerializer.ModelJsonSerializer)
            : Enumerable.Empty<Data.Entity.Animation.Tag>();
          if(!tags.Any()) {
          ...
          /// Return
          return new Data.Entity.Animation.Type("", config, null, tags);
        }
